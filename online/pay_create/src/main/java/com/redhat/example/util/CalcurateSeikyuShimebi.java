...
public class CalcurateSeikyuShimebi {
    ...
    public String getPaydate(String shimebi) {
        // CalcurateDateUtil
        CalcurateDateUtil cal_date_util = new CalcurateDateUtil();

        // 翌月算出
        String yoku_shimebi = cal_date_util.string_convert(
            cal_date_util.getMonthAddDate(cal_date_util.date_convert(shimebi), 1)
        );

        // 10日設定
        return yoku_shimebi.substring(0,6) + "10";
    }
}
